...
A `Map` is very much like its counterpart in Java - it is a collection that maps _keys_ to _values_. The keys must form a set and in most cases are unordered. Here is how to create a basic map:
...
Note that, like their immutable cousins, `+=` and `-=` both return a result of type `Map`. In this case, however, the return value is _the same object_ that we called the method on. The return value is useful for chaining method calls together, but we can discard it if we see fit.
...
Update `favoriteColor` to return a person's favorite color _or_ beige as a default.
